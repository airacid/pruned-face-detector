...
class trainner():
    ...
    def tower_loss(self, scope, images, labels, boxes, L2_reg, training):
        ...
        for num,v in enumerate(endpoint):
            l1_value = tf.reduce_mean(v)
            l1_loss.append(l1_value)
    ...
    def build(self):
        ...
        with self._graph.as_default(), tf.device('/cpu:0'):
            ...
            # # Add feature map sparsity / flops calculation
            # featmap_sparsity = []
            # featmap_size = []
            #
            # channel_sparsity = []
            ...
            #
            # for v in endpoint:
            #     featmap_size.append(tf.size(v))
            #     featmap_sparsity.append(tf.count_nonzero(v, dtype=tf.int32))
            #     channel_max = tf.reduce_max(v, axis=[1,2])
            #     channel_sparsity.append(tf.count_nonzero(channel_max, dtype=tf.int32))
            #     channel_size.append(tf.size(channel_max))
            #
            # self.endpoint = endpoint
            # featmap_size = tf.add_n(featmap_size)
            # featmap_sparsity = tf.add_n(featmap_sparsity)
            # channel_size = tf.add_n(channel_size)
            # channel_sparsity = tf.add_n(channel_sparsity)
            #
            # total_featmap_sparsity = tf.divide(featmap_sparsity, featmap_size)
            # total_channel_sparsity = tf.divide(channel_sparsity, channel_size)
            # self.add_summary(tf.summary.scalar('sparsity/featuremap_sparsity', total_featmap_sparsity))
            # self.add_summary(tf.summary.scalar('sparsity/channel_sparsity', total_channel_sparsity))
            # ema
            if self.ema_weights:
    ...
    def calculate_pruned_flops(self, endpoint):
        ...
        # print(int(flops)//len(max_endpoint[0])/127822233600 *100)
            ...
            for epoch in range(cfg.TRAIN.epoch):
                ...
                if min_loss_control > val_loss:
                    ...
                    low_loss_model_name = cfg.MODEL.model_path + '/optimized/' + \
                                          'epoch_' + str(epoch) + \
                                          'val_loss_' + str(min_loss_control) + '.ckpt'
            ...
            if cfg.TRAIN.vis:
                ...
                _, total_loss_value, reg_loss_value, cla_loss_value, l2_loss_value,pruned_loss_value, endpoint_value, lr_value = \
                    self.sess.run([*self.outputs],
                                  feed_dict=feed_dict)
                # self.calculate_flops(endpoint_value)
                ...
                if self.ite_num % cfg.TRAIN.log_interval == 0:
                    ...
                    mac, avg_channel = self.calculate_pruned_flops(endpoint_value)
