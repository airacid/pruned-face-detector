...
ap.add_argument( "--model", required=False, default='./face_detector/optimized/detector.pb', help="model to eval:")
ap.add_argument( "--is_show", required=False, default=False, help="show result or not?")
ap.add_argument( "--data_dir", required=False, default="/content/WIDER_FACE/WIDER_val", help="dir to img")
ap.add_argument( "--result", required=True,default='./face_detector/result',help="dir to write result")
args = ap.parse_args()
...
def get_data():
    ...
    if subset is 'val':
        wider_face = sio.loadmat(
            './tools/ground_truth/wider_face_val.mat')
    else:
        wider_face = sio.loadmat(
            './tools/ground_truth/wider_face_test.mat')
...
for index, event in enumerate(event_list):
        ...
        for num, file in enumerate(filelist):
            ...
            if os.path.exists(os.path.join(save_path, event[0][0], im_name + '.txt')):
                continue
            ...
            max_im_shrink = np.sqrt(
                1000 * 1000 / (img.shape[0] * img.shape[1]))
